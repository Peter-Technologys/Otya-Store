...
async function requireAdmin(req: NextRequest) {
  ...
  if (!await verifyAdminSession(req, envMap)) return null
  return envMap
}

export async function POST(req: NextRequest) {
  const envMap = await requireAdmin(req)
  if (!envMap) return errorJson('Unauthorized', 401)
  ...
  if (!title || typeof title !== 'string' || !title.trim()) return errorJson('title is required')
  if (!content || typeof content !== 'string' || !content.trim()) return errorJson('content is required')

  const slug = rawSlug && typeof rawSlug === 'string' && rawSlug.trim() ? rawSlug.trim() : slugify(title)
  ...
  try {
    ...
    if (String(err).includes('UNIQUE')) return errorJson('A post with this slug already exists', 409)
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    await db.prepare(`UPDATE blog_posts SET ${setClauses.join(', ')} WHERE id = ?`).bind(...binds).run()
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    if (id) {
      await db.prepare("UPDATE blog_posts SET isPublished = 0, updatedAt = datetime('now') WHERE id = ?").bind(id).run()
    } else {
      await db.prepare("UPDATE blog_posts SET isPublished = 0, updatedAt = datetime('now') WHERE slug = ?").bind(slug).run()
    }
