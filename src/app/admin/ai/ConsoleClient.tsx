...
const starterPrompts = [
  'Check the whole system and tell me what needs my attention.',
  'What is still blocking the next OTYA release?',
  'Check my important operational messages and summarize them.',
  'Give me a short owner update for today.',
]
...
export default function ConsoleClient() {
  ...
  async function sendText(text:string){
    ...
    setInput('');setMessages(m=>[...m,{role:'user',content:message}]);setBusy(true);setStage(0);setNotice('');setActivity('')
    try{
      const d=await api('/api/admin/ai-console/chat',{method:'POST',body:JSON.stringify({message,conversation_id:current||undefined})})
  ...
  if(session.loading || !session.authenticated)return <main className="min-h-screen grid place-items-center" style={{background:'var(--cosmos-scaffold)',color:'var(--cosmos-text-primary)'}}><div className="text-center"><OtyaBrandMark size={54} label="Checking OTYA owner access"/><p className="mt-3 text-sm otya-muted">Opening your secure OTYA workspace…</p></div></main>
  ...
  return <main className="h-[100dvh] overflow-hidden flex" style={{background:'var(--cosmos-scaffold)',color:'var(--cosmos-text-primary)'}}>
    ...
    <aside className={`${sidebar?'translate-x-0':'-translate-x-full'} md:translate-x-0 fixed md:static inset-y-0 left-0 z-40 w-[270px] shrink-0 border-r flex flex-col transition-transform`} style={{background:'var(--cosmos-surface)',borderColor:'var(--cosmos-divider)'}}>
      <div className="h-16 px-4 flex items-center gap-2 border-b" style={{borderColor:'var(--cosmos-divider)'}}><OtyaBrandMark size={34}/><span className="font-black tracking-[-.03em]">tya</span><span className="ml-1 text-xs otya-muted">Console</span><button className="ml-auto md:hidden" aria-label="Close navigation" onClick={()=>setSidebar(false)}>×</button></div>
    ...
    <section className="flex-1 min-w-0 flex flex-col">
      <header className="h-16 shrink-0 border-b flex items-center px-3 sm:px-5" style={{background:'var(--cosmos-app-bar)',borderColor:'var(--cosmos-divider)'}}><button className="md:hidden min-w-11 min-h-11 rounded-xl mr-1" onClick={()=>setSidebar(true)} aria-label="Open navigation">☰</button><OtyaAiMark size={32} active={busy} label={busy?'OTYA AI is working':'OTYA AI'}/><div className="ml-2"><div className="font-black leading-none">OTYA AI</div><div className="text-[11px] otya-muted mt-1">{busy?stages[stage]:'Your owner assistant'}</div></div></header>
      ...
      <div ref={scrollRef} className="flex-1 overflow-auto px-4 sm:px-8 py-6">
        <div className="mx-auto max-w-3xl">
          {messages.length===0?<div className="pt-[10vh] max-w-2xl"><OtyaAiMark size={56}/><h1 className="mt-5 text-3xl sm:text-5xl font-black tracking-[-.055em]">How can I help with OTYA?</h1><p className="mt-3 otya-muted leading-7">Talk to me naturally. I can answer questions, inspect the systems you have connected, explain what is happening, prepare safe changes, help with releases and communications, and ask before sensitive actions.</p><div className="mt-6 grid gap-2 sm:grid-cols-2">{starterPrompts.map(prompt=><button key={prompt} onClick={()=>void sendText(prompt)} className="rounded-2xl border px-4 py-3 text-left text-sm leading-5 hover:bg-black/[.025] dark:hover:bg-white/[.04]" style={{borderColor:'var(--cosmos-divider)'}}>{prompt}</button>)}</div></div>:<div className="space-y-7">{messages.map((m,i)=>m.role==='user'?<div key={i} className="ml-auto max-w-[86%] rounded-[22px] rounded-br-md px-4 py-3 bg-black/[.055] dark:bg-white/[.08] text-sm leading-6">{m.content}</div>:<div key={i} className="max-w-[98%]"><div className="mb-2"><OtyaAiMark size={28}/></div><div className="whitespace-pre-wrap text-sm sm:text-[15px] leading-7">{m.content}</div></div>)}{busy&&<div className="flex items-start gap-3 py-2" role="status" aria-live="polite"><OtyaAiMark size={36} active label="OTYA AI is working"/><div><div className="text-sm font-semibold">{stages[stage]}</div><div className="text-xs otya-muted mt-1">I’ll show useful progress and results without exposing private model reasoning.</div></div></div>}{activity&&!busy&&<div className="text-xs otya-muted border-l-2 pl-3" style={{borderColor:'var(--cosmos-divider)'}}>{activity}</div>}</div>}
        </div>
      </div>
      <div className="shrink-0 p-3 sm:p-5"><div className="mx-auto max-w-3xl"><div className="flex items-end gap-2 rounded-[22px] border p-1.5" style={{background:'var(--cosmos-card)',borderColor:'var(--cosmos-divider)'}}><textarea rows={1} value={input} onChange={e=>setInput(e.target.value)} onKeyDown={composerKey} placeholder="Ask anything about OTYA or tell me what you need" className="max-h-36 min-h-12 flex-1 resize-none bg-transparent px-3 py-3 outline-none text-sm"/><button onClick={()=>void send()} disabled={busy||!input.trim()} className="cosmos-button min-h-12 min-w-12 rounded-[17px] font-black disabled:opacity-50" aria-label="Send">↑</button></div><div className="mt-2 text-center text-[10px] otya-muted">Your account permissions are enforced by the server. Sensitive actions require the appropriate owner authorization.</div></div></div>
    </section>
  </main>
}
