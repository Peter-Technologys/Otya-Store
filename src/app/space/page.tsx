...
export default function SpaceHomePage() {
  ...
  const accountHref = scopedHref(user, 'account', '/account/overview/')
  const methodsHref = scopedHref(user, 'account/sign-in-methods', '/account/sign-in-methods/')
  const devicesHref = scopedHref(user, 'devices', '/account/devices/')
  const storageHref = scopedHref(user, 'storage', '/account/storage/')
  ...
  return <main className="px-4 sm:px-7 lg:px-10 py-7 sm:py-9 max-w-[1240px]">
    ...
    <section className="mt-7 grid md:grid-cols-2 xl:grid-cols-3 gap-4">
      ...
      <SpaceCard title="Playlist recovery" eyebrow="Google Drive" action={<Link href={storageHref} className="text-xs font-black otya-muted">Open</Link>}>
