  ...
  return {
    async send(message) {
      ...
      const response = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: `OTYA Player <${OTYA_NOREPLY_EMAIL}>`,
          to,
...
function withProductionAdapters(env) {
  return { ...env, EMAIL: createResendEmailAdapter(env) }
}
...
function writeRequestAnalytics(env, request, response, startedAt) {
  ...
  try {
    ...
    env.OTYA_ANALYTICS.writeDataPoint({
      blobs: ['request', request.method, analyticsPath(url.pathname), request.cf?.colo ?? 'unknown'],
...
async function readHealthIncident(env) {
  try { return await env.KV?.get?.(HEALTH_INCIDENT_KEY) ?? null } catch { return null }
}
async function writeHealthIncident(env, value) {
  try { await env.KV?.put?.(HEALTH_INCIDENT_KEY, value, { expirationTtl: 24 * 60 * 60 }) } catch {}
}
async function clearHealthIncident(env) {
  try { await env.KV?.delete?.(HEALTH_INCIDENT_KEY) } catch {}
}

async function sendHealthEmail(env, subject, text) {
  try {
    await env.EMAIL.send({
      from: { email: OTYA_NOREPLY_EMAIL, name: 'OTYA Backend' },
...
async function runProductionHealthCheck(env, ctx) {
  ...
  for (const path of HEALTH_PATHS) {
    ...
    try {
      const request = new Request(new URL(path, baseUrl), { method: 'GET', headers: { 'User-Agent': 'OTYA-Internal-HealthCheck/2.0' } })
  ...
  if (down.length === 0) {
    ...
    if (previousIncident) {
      ...
      await sendHealthEmail(env, '[OTYA Backend] Production routes recovered', [
  ...
  await sendHealthEmail(env, `[OTYA Backend] ${down.length} production route(s) unhealthy`, [
    'OTYA production route health alert', '',
    ...down.map((result) => `${result.path} — status ${result.status}, latency ${result.latency}ms${result.error ? ` (${result.error})` : ''}`), '',
    `Checked at: ${new Date().toISOString()}`,
  ].join('\n'))
}
...
export default {
    ...
    if (url.pathname === '/api/admin/release-workflow' && request.method === 'POST') {
      if (!isAdmin(request, runtimeEnv)) response = json({ error: 'Unauthorized' }, 401)
        ...
        try {
          ...
          response = json({ error: 'Could not start release workflow', detail: error instanceof Error ? error.message : 'Invalid request' }, 400)
        }
      }
      writeRequestAnalytics(runtimeEnv, request, response, startedAt)
      return response
    ...
    if (url.pathname === '/api/admin/release-workflow/status' && request.method === 'GET') {
      if (!isAdmin(request, runtimeEnv)) response = json({ error: 'Unauthorized' }, 401)
        ...
        if (!id) response = json({ error: 'id is required' }, 400)
          ...
          } catch { response = json({ error: 'Workflow instance not found' }, 404) }
        }
      }
      writeRequestAnalytics(runtimeEnv, request, response, startedAt)
      return response
  ...
  async scheduled(event, env, ctx) {
    const runtimeEnv = withProductionAdapters(env)
    if (event.cron === HEALTH_CHECK_CRON) return runProductionHealthCheck(runtimeEnv, ctx)
