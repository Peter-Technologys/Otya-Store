...
function isAdmin(request, env) { if (!env.ADMIN_TOKEN) return false; const header=request.headers.get('Authorization')??''; return header.startsWith('Bearer ')&&header.slice(7)===env.ADMIN_TOKEN }
...
function analyticsPath(pathname){if(pathname.startsWith('/auth/'))return'/auth/*';if(pathname.startsWith('/apk/'))return'/apk/*';if(pathname.startsWith('/api/admin/'))return'/api/admin/*';if(pathname.startsWith('/api/telegram/'))return'/api/telegram/*';if(pathname.startsWith('/api/'))return'/api/*';return pathname==='/'?'/':'/other'}
...
export default {
 ...
 async fetch(request,env,ctx){
  ...
  if(url.pathname==='/api/ai/chat'){
    if(!runtimeEnv.AI_SUPPORT?.fetch)response=json({error:'AI service unavailable'},503)
  ...
  if(url.pathname==='/api/admin/release-workflow'&&request.method==='POST'){if(!isAdmin(request,runtimeEnv))response=json({error:'Unauthorized'},401);else if(!runtimeEnv.OTYA_RELEASE_WORKFLOW?.create)response=json({error:'Release workflow binding unavailable'},503);else{try{const params=await request.json();const instance=await runtimeEnv.OTYA_RELEASE_WORKFLOW.create({params});response=json({ok:true,instanceId:instance.id,status:'queued'},202)}catch(error){response=json({error:'Could not start release workflow',detail:error instanceof Error?error.message:'Invalid request'},400)}}writeRequestAnalytics(runtimeEnv,request,response,startedAt);return response}
  if(url.pathname==='/api/admin/release-workflow/status'&&request.method==='GET'){if(!isAdmin(request,runtimeEnv))response=json({error:'Unauthorized'},401);else if(!runtimeEnv.OTYA_RELEASE_WORKFLOW?.get)response=json({error:'Release workflow binding unavailable'},503);else{const id=url.searchParams.get('id');if(!id)response=json({error:'id is required'},400);else{try{const instance=await runtimeEnv.OTYA_RELEASE_WORKFLOW.get(id);response=json({ok:true,instanceId:id,workflow:await instance.status()})}catch{response=json({error:'Workflow instance not found'},404)}}}writeRequestAnalytics(runtimeEnv,request,response,startedAt);return response}
