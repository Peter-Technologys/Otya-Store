    ...
    <nav className="sticky top-0 z-50 border-b backdrop-blur-xl" style={{ borderColor: 'var(--border)', background: 'rgba(255,255,255,0.92)' }}>
      <div className="max-w-6xl mx-auto px-6 py-3 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2.5 shrink-0">
          <Image src="/web-app-manifest-192x192.png" alt="PeterSmart Link" width={36} height={36} className="rounded-xl object-cover shadow-sm" />
          <div>
        ...
        <div className="hidden md:flex items-center gap-1">
          {NAV_LINKS.map(l => (
            <a key={l.href} href={l.href} target={l.href.startsWith('http') ? '_blank' : undefined} rel={l.href.startsWith('http') ? 'noopener noreferrer' : undefined}
              className="px-3 py-2 rounded-lg text-sm font-medium transition-colors hover:bg-purple-50" style={{ color: 'var(--text-sub)' }}>
              {l.label}
            </a>
          ))}
        </div>
        <div className="flex items-center gap-2">
          ...
          <a href="https://wa.me/256775912582" target="_blank" rel="noopener noreferrer"
            className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-green-500 hover:bg-green-400 text-white text-xs font-semibold transition-all">
            WhatsApp
          </a>
          <button onClick={() => setOpen(!open)} className="md:hidden flex items-center justify-center w-9 h-9 rounded-lg hover:bg-gray-100 transition-colors" aria-label="Toggle menu">
      ...
      {open && (
        <div className="md:hidden border-t px-6 py-4 space-y-1" style={{ borderColor: 'var(--border)', background: 'var(--bg)' }}>
          {NAV_LINKS.map(l => (
            <a key={l.href} href={l.href} target={l.href.startsWith('http') ? '_blank' : undefined} rel={l.href.startsWith('http') ? 'noopener noreferrer' : undefined}
              onClick={() => setOpen(false)} className="block px-4 py-2.5 rounded-xl text-sm font-medium transition-colors hover:bg-purple-50" style={{ color: 'var(--text)' }}>
              {l.label}
            </a>
          ))}
          <a href="https://wa.me/256775912582" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 mt-2 px-4 py-2.5 rounded-xl bg-green-500 text-white text-sm font-semibold">WhatsApp Us</a>
        </div>
      )}
    </nav>
  )
