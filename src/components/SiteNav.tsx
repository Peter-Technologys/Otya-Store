    ...
    <nav className="sticky top-0 z-50 border-b backdrop-blur-xl" style={{ borderColor: 'var(--cosmos-divider)', background: 'var(--nav-bg)' }}>
      ...
      {open && <div className="md:hidden border-t" style={{ borderColor: 'var(--cosmos-divider)', background: 'var(--cosmos-app-bar)' }}>
        <div className="px-3 py-2">
          ...
          <Link href="/download/otya-player" onClick={() => setOpen(false)} className="cosmos-button flex justify-center px-4 py-2.5 rounded-lg text-sm font-semibold mt-1">Download OTYA Player</Link>
        </div>
      </div>}
    </nav>
  )
