  ...
  return <>
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <div ref={accountWrap} className="relative">
            ...
            {signedIn&&accountOpen&&<div className="absolute right-0 mt-2 w-[280px] overflow-hidden rounded-[22px] border border-black/[.08] dark:border-white/[.10] bg-[color:var(--cosmos-surface)] shadow-2xl">
              ...
              <div className="p-2 text-sm"><Link href="/account" onClick={()=>setAccountOpen(false)} className="block rounded-xl px-3 py-2.5 font-bold hover:bg-black/[.04] dark:hover:bg-white/[.06]">Otya Space</Link><button onClick={()=>void signOut()} className="w-full text-left rounded-xl px-3 py-2.5 text-red-500 hover:bg-black/[.04] dark:hover:bg-white/[.06]">Sign out</button></div>
            </div>}
          </div>
        </div>
      </div>
    ...
    <div className={`fixed inset-0 z-[70] md:hidden ${menuOpen?'pointer-events-auto':'pointer-events-none'}`} aria-hidden={!menuOpen}>
      ...
      <aside className={`absolute inset-y-0 left-0 w-[86vw] max-w-[340px] border-r border-black/[.08] dark:border-white/[.10] bg-[color:var(--cosmos-surface)] shadow-2xl transition-transform ${menuOpen?'translate-x-0':'-translate-x-full'}`}>
        ...
        <div className="p-4 space-y-1 text-sm">{publicLinks.map(([label,href])=><Link key={href} href={href} onClick={()=>setMenuOpen(false)} className="block rounded-2xl px-3 py-3 font-bold hover:bg-black/[.045] dark:hover:bg-white/[.06]">{label}</Link>)}<Link href="/ask" onClick={()=>setMenuOpen(false)} className="mt-3 flex items-center gap-3 rounded-2xl px-3 py-3 font-black bg-black/[.025] dark:bg-white/[.04]"><OtyaBrandMark ai size={28}/>Next</Link>{signedIn?<><Link href="/account" onClick={()=>setMenuOpen(false)} className="block rounded-2xl px-3 py-3 font-bold">Otya Space</Link><button onClick={()=>void signOut()} className="w-full text-left rounded-2xl px-3 py-3 text-red-500">Sign out</button></>:<Link href="/sign-in" onClick={()=>setMenuOpen(false)} className="cosmos-button mt-4 flex min-h-11 items-center justify-center rounded-full px-4 font-black">Sign in</Link>}</div>
      </aside>
    </div>
  </>
}
