...
export function OtyaAssistPrompt({compact=false}:{compact?:boolean}){
  ...
  return <section className={`${compact?'flex-1 min-h-0':'my-6 h-[min(720px,72dvh)]'} flex flex-col overflow-hidden rounded-[28px] border border-black/[.06] dark:border-white/[.08] bg-white/75 dark:bg-white/[.025] shadow-[0_20px_70px_rgba(20,14,38,.07)] backdrop-blur-xl`}>
    <div className="shrink-0 flex items-center gap-3 px-4 sm:px-5 py-3.5 border-b border-black/[.05] dark:border-white/[.07]">
      <OtyaAiMark size={32} active={loading} label={loading?'Otya AI is working':'Otya AI'}/>
      <div className="min-w-0 flex-1"><div className="font-black tracking-[-.025em]">Otya AI</div><div className="text-[11px] otya-muted">{loading?thinkingStages[thinkingStage]:'Chat naturally. Follow up anytime.'}</div></div>
    ...
    <div ref={scrollRef} className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-4 sm:px-6 py-5" aria-live="polite">
      ...
      :<div className="mx-auto max-w-3xl space-y-6">{messages.map((message,index)=>message.role==='user'?<div key={index} className="ml-auto max-w-[86%] rounded-[22px] rounded-br-md px-4 py-3 text-sm leading-6 bg-black/[.055] dark:bg-white/[.08]">{message.content}</div>:<div key={index} className="max-w-[96%]"><div className="mb-2.5 flex items-center gap-2"><OtyaAiMark size={28}/></div><div className="whitespace-pre-wrap text-sm leading-7">{message.content}</div><div className="mt-2.5 flex flex-wrap gap-2"><button type="button" onClick={()=>void navigator.clipboard.writeText(message.content)} className="min-h-9 rounded-full px-3 text-[11px] font-black otya-muted">Copy</button>{message.failed&&<button type="button" onClick={()=>void retryLast()} disabled={loading} className="min-h-9 rounded-full px-3 text-[11px] font-black">Retry</button>}{message.handoff&&<button type="button" onClick={()=>setShowHandoff(true)} className="min-h-9 rounded-full px-3 text-[11px] font-black">Contact support</button>}</div></div>)}{loading&&<div className="flex items-center gap-3 py-2" role="status" aria-live="polite"><OtyaAiMark size={34} active label="Otya AI is working"/><div><div className="text-sm font-semibold">{thinkingStages[thinkingStage]}</div><div className="text-[11px] otya-muted">Otya may inspect connected information when needed.</div></div></div>}</div>}
    </div>
