...
export function OtyaAssistPrompt({compact=false}:{compact?:boolean}){
  ...
  return <section className={`${compact?'flex-1 min-h-0':'my-6 h-[min(720px,72dvh)]'} flex flex-col overflow-hidden rounded-[28px] border border-black/[.06] dark:border-white/[.08] bg-white/75 dark:bg-white/[.025] shadow-[0_20px_70px_rgba(20,14,38,.07)] backdrop-blur-xl`}>
    ...
    <div ref={scrollRef} className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-4 sm:px-6 py-5" aria-live="polite">
      ...
      :<div className="mx-auto max-w-3xl space-y-6">{messages.map((message,index)=>message.role==='user'?<div key={index} className="ml-auto max-w-[86%] rounded-[22px] rounded-br-md px-4 py-3 text-sm leading-6 bg-black/[.055] dark:bg-white/[.08]">{message.content}</div>:<div key={index} className="max-w-[96%]"><div className="mb-2.5 flex items-center gap-2"><OtyaMark/><span className="text-xs font-black">Otya</span></div><div className="whitespace-pre-wrap text-sm leading-7">{message.content}</div><div className="mt-2.5 flex flex-wrap gap-2"><button type="button" onClick={()=>void navigator.clipboard.writeText(message.content)} className="min-h-9 rounded-full px-3 text-[11px] font-black otya-muted">Copy</button>{message.failed&&<button type="button" onClick={()=>void retryLast()} disabled={loading} className="min-h-9 rounded-full px-3 text-[11px] font-black">Retry</button>}{message.handoff&&<button type="button" onClick={()=>setShowHandoff(true)} className="min-h-9 rounded-full px-3 text-[11px] font-black">Contact support</button>}</div></div>)}{loading&&<div className="flex items-center gap-3 py-2" role="status" aria-label="Otya is thinking"><OtyaMark thinking/><span className="text-sm otya-muted">Thinking…</span></div>}</div>}
    </div>
    ...
    <form onSubmit={submit} className="shrink-0 border-t border-black/[.05] dark:border-white/[.07] p-3 sm:p-4"><div className="mx-auto flex max-w-3xl items-end gap-2 rounded-[20px] border border-black/[.07] dark:border-white/[.09] bg-black/[.018] dark:bg-white/[.018] p-1.5"><textarea value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={onComposerKeyDown} rows={1} placeholder="Ask Otya anything" aria-label="Ask Otya anything" className="max-h-32 min-h-11 min-w-0 flex-1 resize-none bg-transparent px-3 py-2.5 text-sm outline-none"/><button disabled={loading||!query.trim()||!guestId} aria-label="Send message" className="cosmos-button min-h-11 min-w-11 rounded-[15px] px-3 text-lg font-black disabled:opacity-50">↑</button></div></form>
  </section>
}
